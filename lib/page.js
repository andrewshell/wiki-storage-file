...
module.exports = function (argv) {
  ...
  const fs = require('./storage.file.js')(argv);

  function load_parse(file, cb, annotations) {
    if (annotations == null) {
      annotations = {};
    }
    return fs.readFile(file, function(err, data) {
      ...
      try {
        ...
        e = _error;
        return fs.rename(file, `recycler/${file}`, function(err) {
          if (err) {
          ...
          if (exists) {
            ...
            return fs.exists(defloc, function(exists) {
              if (exists) {
                ...
                return fs.glob("wiki-plugin-*/pages", {
                  cwd: argv.packageDir
                }, function(e, plugins) {
